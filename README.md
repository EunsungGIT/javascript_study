...
----------------------------
* 소수점 안나오게 하는 코드 `Math.floor()`
## 스크롤
* deltaY = 세로 스크롤 양 반환하는 마우스 이벤트 속성
    - 아래로 내려갈 때 = 양수 +100씩
    - 위로 올라갈 때 = 음수 -100씩
* menu, anchors 두 속성은 세트 (<a href="#a"> 샵 뒤에 a와 anchors ['a'] 일치해야함)
* 스크롤을 내릴 때 등장 애니메이션을 추가하고 싶은 경우 -> 제이쿼리 함수 추가
    - $(제이쿼리 대상).style.top = '50px' (불가능)
    - $(제이쿼리 대상).get(0).style.top = '50px' (가능)
    - get(index)라는 메서드는 제이쿼리오 자바스크립트를 연결해주는 다리 역할을 함
-----------------------
## swiper
* 태그 구조 지키기
* 태그 클래스명 지키기(추가 클래스명은 자유)
* 태그 부모에 아이디 설정하기 - 해당 아이디 명으로 swiper연결하기
* 태그 부모에 max-width, overflow 등 필요 속성 설정하기
-----------------------
## 비동기함수
* 자바스크립트 개념
    - 자바스크립트는 호출 스택(싱글 쓰레드(single-threaded))이 1개다.
    - 자바스크립트는 작성된 코드를 한 줄 씩 순차적으로 실행한다.(동기식)
    - 위->아래로 순차적으로 실행하기 때문에 절차적 언어라고도 한다.
    - 앞의 한 작업이 끝나야 다음 작업을 실행한다.(blocking)